...
func CreateCustomHandleStruct(url, userid string, param int) string {
...
func CheckCustomHandleExists(db *gorm.DB, handle string) (bool, error) {
	var count int64
	err := db.Model(&datatypes.Entry{}).Where("param = ?", handle).Count(&count).Error
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
func GetEntryByID(db *gorm.DB, id string) (*datatypes.Entry, error) {
	var entry datatypes.Entry
	if err := db.Where("param = ?", id).First(&entry).Error; err != nil {
		return nil, err
	}
	return &entry, nil
}
...
func PatchCustomHandle(db *gorm.DB, auth *auth.Client, rdb *redis.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		id := c.Param("id")
		if id == "" {
			errorPatch(c, errors.New("no id provided"), "Failed to convert id param", 400)
			return
		}
		...
		if err := UpdateCustomHandle(db, json.Handle, entry.ID); err != nil {
		...
		if err := rdb.Set(context.Background(), entry.Param, ":e:"+userid, 0).Err(); err != nil {
